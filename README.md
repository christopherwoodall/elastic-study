...
* **[Dynamic MCP Server (`./docs/MCP.md`)](https://www.google.com/search?q=%5Bhttps://www.google.com/search%3Fq%3D./docs/MCP.md%5D(https://www.google.com/search%3Fq%3D./docs/MCP.md))**: A hybrid Model Context Protocol (MCP) Server built with FastAPI. It provides an interactive Swagger UI for manual testing and a remote Server-Sent Events (SSE) endpoint that grants LLM agents the ability to dynamically introspect Elasticsearch schemas, list indices, and execute raw JSON Query DSL analytics with built-in exponential backoff resiliency.

**Try it out - See [Quick Start](./docs/QUICK_START.md) for instructions**
